...
static size_t
subspace_store_internal_node_subitems(SubspaceStoreInternalNode *node, int index)
{
	...
	{
		SubspaceStoreInternalNode *next = slice->storage;
				   ...
				   void *object, void (*object_free) (void *))
{
	...
	for (i = 0; i < hc->num_slices; i++)
	{
		...
		if (match == NULL)
		{
			...
			if (store->max_items > 0 && i == 0 && node->items >= store->max_items)
			{
				/*
				 ...
				 * the subspace_store.
				 */
				size_t		items_removed = subspace_store_internal_node_subitems(node, 0);

				Assert(store->max_items == node->items);

				dimension_vec_remove_slice(&node->vector, 0);
				node->items -= items_removed;
			}
